...
(function () {
  ...
  const defaultData = {
    recipientName: 'Bejei',
    senderSignature: 'From your TortaSeller BABI ❤️',
...
I hope this little surprise brings the biggest smile to your beautiful face! 🎂🌸🎉`,
    videos: [
      {
        url: 'https://res.cloudinary.com/ydwq0y2h/video/upload/v1789313829/j8beumf9fxvkhiwpaznn.mp4',
  ...
  // ==========================================
  // CLOUDINARY VIDEO UPLOAD & RENDER BACKEND SYNC
  // Everyone who opens the Render link or downloaded HTML will see the video!
  // ==========================================
  const BACKEND_BASE = (window.location.origin && window.location.origin.includes('onrender.com'))
    ? window.location.origin
    : 'https://birthday-surprise-x7jd.onrender.com';
  ...
  function uploadVideoToCloudinary(file, onProgress) {
    return new Promise((resolve, reject) => {
      ...
      formData.append('tags', 'birthday_surprise');
  ...
  async function uploadVideoToServer(file) {
    try {
      const res = await fetch(`${BACKEND_BASE}/api/upload-video`, { method: 'POST', body: file });
      if (res.ok) {
  ...
  function applyVideoOrientation(container, isPortrait, ratio, vw, vh, isCompact) {
    ...
    if (isPortrait) {
      ...
      if (orientationToggleLabel) orientationToggleLabel.innerHTML = 'Switch to Landscape 💻';
    } else {
      ...
      if (orientationToggleLabel) orientationToggleLabel.innerHTML = 'Switch to Portrait 📱';
    }
  }
  ...
  function renderVideoPlayer(targetEl, videoData, isCompact) {
    ...
    if (!parsed) {
      targetEl.innerHTML = '<div class="video-placeholder-container">' +
        ...
        '<span style="font-size:0.85rem; opacity:0.85;">Add a YouTube URL or video file in Creator Studio! 🌸</span>' +
        '</div>';
      return;
    }
    ...
    if (parsed.type === 'youtube' || parsed.type === 'vimeo' || parsed.type === 'drive') {
      ...
      video.preload = 'auto';
      if (parsed.src && parsed.src.includes('res.cloudinary.com')) {
        video.poster = parsed.src.replace(/\.[^/.]+$/, '.jpg');
      }
      ...
      video.addEventListener('error', () => {
        ...
        targetEl.innerHTML = `
          <div class="video-placeholder-container" style="padding:1.5rem; text-align:center;">
            <div class="video-placeholder-icon" style="color:var(--primary-pink); font-size:2rem; margin-bottom:0.5rem;"><i class="fa-solid fa-circle-exclamation"></i></div>
            <strong style="font-size:1rem; margin-bottom:0.3rem; display:block; color:var(--text-heading);">Video Not Found or Expired</strong>
            <span style="font-size:0.82rem; opacity:0.85; display:block; margin-bottom:0.8rem; color:var(--text-muted);">The video link is unavailable. You can easily upload or paste a fresh link in Creator Studio!</span>
            <button type="button" class="btn btn-outline-small" onclick="document.getElementById('btn-customize').click()" style="font-size:0.78rem; padding:0.4rem 0.8rem; border-radius:12px;"><i class="fa-solid fa-wand-magic-sparkles"></i> Open Creator Studio</button>
          </div>
        `;
      });
    ...
    customizerVideosList.forEach((item, index) => {
      ...
      if (fileInput) {
        fileInput.addEventListener('change', async (e) => {
          ...
          // Immediately save to IndexedDB as offline safety net
          saveVideoToIndexedDB(file).catch(err => console.warn('IndexedDB save warning:', err));
          ...
          try {
            ...
            // Immediately persist in local state
            if (appData.videos && appData.videos[index]) {
              appData.videos[index].url = cloudinaryUrl;
            }
            if (index === 0 && appData.featuredVideo) {
              appData.featuredVideo.url = cloudinaryUrl;
            }
            try {
            ...
            if (uploadStatus) {
              uploadStatus.innerHTML = `
                <div style="color:#2e7d32; display:flex; align-items:center; gap:6px; margin-top:4px;">
                  <i class="fa-solid fa-circle-check"></i> <strong>Uploaded to Cloudinary! ✨</strong> Link saved permanently!
                </div>
              `;
            }
          } catch (err) {
            ...
            if (uploadStatus) {
              uploadStatus.innerHTML = `
                <div style="color:#d81458; margin-top:4px;">
                  ⚠️ Cloud upload note: ${err.message}. Saved securely to your browser offline storage!
                </div>
              `;
            }
          }
  ...
  btnSaveCustomizer.addEventListener('click', async () => {
    ...
    try {
      await fetch(`${BACKEND_BASE}/api/surprise`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
  ...
  // Download self-contained HTML with custom video embedded permanently
  ...
  if (btnDownloadHtml) {
    btnDownloadHtml.addEventListener('click', () => {
      ...
      // Bake current appData directly into const defaultData in the downloaded HTML
      const serializedData = JSON.stringify(appData, null, 2);
      htmlContent = htmlContent.replace(/const defaultData = \{[\s\S]*?\n  \};/, `const defaultData = ${serializedData};`);
  ...
  async function loadInitialData() {
    ...
    try {
      const res = await fetch(`${BACKEND_BASE}/api/surprise`);
      if (res.ok) {
        const json = await res.json();
        if (json && json.data && json.data.recipientName) {
          const preservedVideos = (appData.videos && appData.videos.length > 0 && !appData.videos[0].url.includes('nl62hhiBMOM')) ? appData.videos : null;
          const preservedFeatured = (appData.featuredVideo && appData.featuredVideo.url && !appData.featuredVideo.url.includes('nl62hhiBMOM')) ? appData.featuredVideo : null;
          
          appData = Object.assign({}, defaultData, json.data);
          
          // If server has valid videos, prioritize server videos
          if (json.data.videos && Array.isArray(json.data.videos) && json.data.videos.length > 0 && json.data.videos[0].url) {
            appData.videos = json.data.videos;
            appData.featuredVideo = json.data.featuredVideo || json.data.videos[0];
          } else if (preservedVideos) {
            appData.videos = preservedVideos;
            appData.featuredVideo = preservedFeatured || preservedVideos[0];
          }
      ...
      try {
        ...
        if (cloudRes.ok) {
          ...
          if (cloudData && cloudData.recipientName) {
            ...
            if (cloudData.videos && Array.isArray(cloudData.videos) && cloudData.videos.length > 0 && cloudData.videos[0].url) {
              appData.videos = cloudData.videos;
              appData.featuredVideo = cloudData.featuredVideo || cloudData.videos[0];
            } else if (preservedVideos) {
              appData.videos = preservedVideos;
              appData.featuredVideo = preservedFeatured || preservedVideos[0];
            }
    ...
    if (videoParam && videoParam.trim()) {
      ...
      if (!appData.videos || !appData.videos.length) appData.videos = [appData.featuredVideo];
      else appData.videos[0].url = videoParam.trim();
    }
    ...
    try {
      ...
      try {
        const headRes = await fetch(`${BACKEND_BASE}/api/video`, { method: 'HEAD' });
        if (headRes.ok) {
          if (!appData.featuredVideo) appData.featuredVideo = Object.assign({}, defaultData.featuredVideo);
          appData.featuredVideo.url = `${BACKEND_BASE}/api/video`;
          if (appData.videos && appData.videos[0]) appData.videos[0].url = `${BACKEND_BASE}/api/video`;
          hasLoadedVideo = true;
        }
      } catch (e) {}

      // Only check IndexedDB if no video has been loaded yet or if current video is a placeholder
      if (!hasLoadedVideo && (!appData.featuredVideo || !appData.featuredVideo.url || appData.featuredVideo.url.includes('nl62hhiBMOM'))) {
        ...
        if (storedBlob) {
          ...
          if (appData.videos && appData.videos[0]) appData.videos[0].url = persistentUrl;
        }
      }
    } catch (e) {}
